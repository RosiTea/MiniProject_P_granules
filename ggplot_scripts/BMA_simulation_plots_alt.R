...
alt_model <- read_csv("20240305_data/AlternativeModel.csv")
...
ggplot(am_long, aes(x = Time, y = Amount, colour = Name, group = Name)) + 
  geom_line() +
  theme_minimal() +
  labs(
...
am_post_subset <- am_long %>% 
  filter(Name %in% post_proteins)
am_ant_subset <- am_long %>% 
  filter(Name %in% ant_proteins)
am_g_subset <- am_long %>% 
  filter(Name %in% granule_proteins)

# Now plot the filtered data
ggplot(am_post_subset, aes(x = Time, y = Amount, colour = Name, group = Name, linetype = Name)) + 
  geom_line(size=1.5) +
  theme_minimal() +
  labs(
...
ggplot(am_ant_subset, aes(x = Time, y = Amount, colour = Name, group = Name, linetype = Name)) + 
  geom_line(size=1.5) +
  theme_minimal() +
  labs(
...
ggplot(am_g_subset, aes(x = Time, y = Amount, colour = Name, group = Name)) + 
  geom_line(size=1.5) +
  theme_minimal() +
  labs(
